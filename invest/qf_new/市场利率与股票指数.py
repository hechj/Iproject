...
def draw_rate(long_rate):
    ...
    if long_rate:
        ...
        ax2.plot(df['3m'], label="利率走势", linestyle='-', linewidth=3, color='skyblue', alpha=1)
...
if __name__ == '__main__':
    ...
    draw_rate(True)
    ax1.plot(df_300['close'], label='沪深300指数走势', linestyle='-', color='salmon', linewidth=2, alpha=0.6)
    ...
    ax1.add_patch(
        plt.Rectangle(((pd.to_datetime("2014-06-01") - pd.to_datetime("1970-01-01")).days, 1000), 360,
                      4000, fill=False, edgecolor='black', linewidth=1, linestyle='--', clip_on=False)
    )
    ax1.add_patch(
        plt.Rectangle(((pd.to_datetime("2018-12-01") - pd.to_datetime("1970-01-01")).days, 1000), 360,
                      4000, fill=False, edgecolor='black', linewidth=1, linestyle='--', clip_on=False)
    )
    ax1.text(x="2012-01-01", y=1000, s='hs300', size=12)
    plt.savefig("市场利率.jpg")
    plt.show()
    plt.close()
