...
dict_code = {'000001.SZ': '平安银行', '601166.SH': '兴业银行', '600036.SH': '招商银行'}
...
def draw():
    ...
    # 数据切片，从**年开始取
    data = data[startdate::]
    ax2.plot(data.index, data["收盘"], label='十年期国债收益率(%)', linestyle='-', linewidth=1)
...
def drawline(tcode, color):
    ...
    ax1.plot(stockline, datadf.close, label=dict_code[datadf.ts_code[0]], linestyle='-', linewidth=1, color=color,
             alpha=1)
...
if __name__ == '__main__':
    ...
    drawline('601166.SH', color='purple')
    drawline('000001.SZ', color='red')
    drawline('600036.SH', color='darkgreen')
    ...
    plt.title("国债与银行股的关系", fontsize=20, fontproperties=myfont, bbox=dict(facecolor='blue', alpha=0.1))
